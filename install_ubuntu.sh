...
do
    case "${_PARAMETER}" in
      ...
      ;;
      --from-package|-p)
        echo "Sorry, install from-package is not available for Ubuntu."
...
# 更新包列表并安装必要的依赖包
sudo apt-get update
sudo apt-get install -y apache2-utils libpam-pwdfile

apt-get update
apt-get install gcc g++ make vim libpam-dev libwrap0-dev unzip httpd-tools -y
...
${CGREEN} Socks5 User&Passwd:  $CMAGENTA ${DEFAULT_USER}:${DEFAULT_PAWD}$CEND
	echo "$CMAGENTA ${DEFAULT_USER}:${DEFAULT_PAWD}"
