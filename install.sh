...
REQUEST_SERVER="https://raw.github.com/wuhcun666/danted/master"
SCRIPT_SERVER="https://public.sockd.info"
SYSTEM_RECOGNIZE=""
...
if [ -s "/etc/os-release" ];then
    ...
    if [ -n "$(echo ${os_name} | grep -Ei 'Debian' )" ];then
        printf "Current OS: %s\n" "${os_name}"
        SYSTEM_RECOGNIZE="debian"
	elif [ -n "$(echo ${os_name} | grep -Ei 'Ubuntu')" ];then
        printf " ---- Current OS: %s\n" "${os_name}"
        SYSTEM_RECOGNIZE="ubuntu"
